...
def print_new_sentence():
  ...
  for t in range(len(props)):
    ...
    # print(words)
    n_words = [word.decode('ascii') if type(word)!=str else word for word in words]
    # n_temp = [type(each) for each in tags[t]]
    # print(n_temp)
    # print(str(props[t]))
    # print(" ".join(tags[t]))
    # print(type(" ".join(n_words)))
    # print(str(" ".join(n_words).encode('ascii')))
    # print(type(str(props[t])))
    # print(" ".join(n_words).encode('ascii'))
    # print(type(" ".join(tags[t])))
    fout.write(str(props[t]) + " " + " ".join(n_words) + " ||| " + " ".join(tags[t]) + "\n")
    propid_labels[props[t]] = 'V'
    fd_out.write(domain + '\n')
  
  n_words = [word.decode('ascii') if type(word)!=str else word for word in words]
  fout_propid.write(" ".join(n_words) + " ||| " + " ".join(propid_labels) + "\n")
